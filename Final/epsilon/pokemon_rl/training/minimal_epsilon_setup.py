...
def _sanitize_city_swarm_targets(value) -> list[dict]:
    if not value:
        return []
    sanitized: list[dict] = []
    for entry in value:
        ...
        if not name:
            continue
        label = entry.get("label")
        cleaned_label = str(label).strip() if isinstance(label, str) and label else None
        try:
            episodes = max(1, int(entry.get("episodes", 1)))
        except (TypeError, ValueError):
            episodes = 1
        requires_badges = [badge.lower() for badge in _coerce_str_list(entry.get("requires_badges", []))]
        requires_flags = [flag.lower() for flag in _coerce_str_list(entry.get("requires_story_flags", []))]
        sanitized.append(
            {
                "name": name,
                "label": cleaned_label,
                "episodes": episodes,
                "requires_badges": requires_badges,
                "requires_story_flags": requires_flags,
            }
        )
    return sanitized
...
class TrainConfig:
    ...
    curriculum_swarm_enabled: bool
    city_swarm_targets: list[dict]
    render_map: bool
    map_refresh: int
    map_viz_fps: float
...
class CurriculumManager:
    def __init__(
        ...
        *,
        ...
        self.num_envs = num_envs
        self.swarm_mode = bool(swarm_mode)
        self.swarm_stride = num_envs if self.swarm_mode else 1
        sequence_len = len(self.sequence)
        if sequence_len > 0 and self.swarm_mode:
            self.indices = [idx % sequence_len for idx in range(num_envs)]
        else:
    ...
    def assign(
        ...
        for _ in range(len(self.sequence)):
            ...
            step = self.swarm_stride if (self.swarm_mode and self.sequence) else 1
            self.indices[env_idx] = (idx + step) % len(self.sequence)
...
def parse_args():
    ...
    parser.add_argument(
        "--curriculum-swarm",
        action="store_true",
        dest="curriculum_swarm_enabled",
        help="Enable curriculum swarming so each env boots from a different savestate/city.",
    )
    parser.add_argument(
        "--no-curriculum-swarm",
        action="store_false",
        dest="curriculum_swarm_enabled",
        help="Disable curriculum swarming and use classic round-robin savestates per env.",
    )
    parser.set_defaults(curriculum_swarm_enabled=None)
    parser.add_argument(
    ...
    additional_defaults = {
        ...
        "curriculum_swarm_enabled": False,
    }
    list_defaults = {
        "key_item_ids": [],
        "pokedex_milestones": [],
...
def build_config(args) -> TrainConfig:
    ...
    city_swarm_targets = _sanitize_city_swarm_targets(
        getattr(args, "city_swarm_targets", []) or []
    )
    ...
    curriculum_swarm_enabled = getattr(args, "curriculum_swarm_enabled", None)
    if curriculum_swarm_enabled is None:
        curriculum_swarm_enabled = False
    ...
    curriculum_swarm_enabled = bool(curriculum_swarm_enabled)
    return TrainConfig(
        rom_path=args.rom,
        ...
        curriculum_swarm_enabled=curriculum_swarm_enabled,
        city_swarm_targets=city_swarm_targets,
...
def run_replay(
    ...
    curriculum_manager = (
        CurriculumManager(
            cfg.curriculum_states,
            cfg.state_path,
            num_envs,
            swarm_mode=cfg.curriculum_swarm_enabled,
        )
...
def train(cfg: TrainConfig) -> None:
    ...
    city_swarm_lookup: dict[str, dict] = {}
    if cfg.city_swarm_targets:
        for target in cfg.city_swarm_targets:
            name = str(target.get("name") or "").strip()
            if not name:
                continue
            slug = _slugify(name)
            requires_badges = target.get("requires_badges") or []
            requires_story_flags = target.get("requires_story_flags") or []
            label = target.get("label") or f"new_town_{slug}"
            episodes = max(1, int(target.get("episodes", cfg.auto_curriculum_capture_episodes)))
            city_swarm_lookup[slug] = {
                "label": label,
                "requires_badges": requires_badges,
                "requires_story_flags": requires_story_flags,
                "episodes": episodes,
            }
            capture_map_whitelist.add(name.lower())
            capture_map_whitelist.add(slug.replace("_", " "))
    if cfg.auto_curriculum_capture:
    ...
    def _capture_event_state(
        ...
        if _capture_savestate(envs[env_idx], destination):
            ...
            if curriculum_manager:
                req_badges = None
                req_flags = None
                label_for_manager = label
                episodes_override = cfg.auto_curriculum_capture_episodes
                ...
                elif normalized.startswith("new_town_"):
                    slug = normalized.split("new_town_", 1)[1]
                    meta = city_swarm_lookup.get(slug)
                    if meta:
                        req_badges = meta.get("requires_badges")
                        req_flags = meta.get("requires_story_flags")
                        episodes_override = meta.get(
                            "episodes", cfg.auto_curriculum_capture_episodes
                        )
                        label_for_manager = meta.get("label") or label
    ...
    def _bootstrap_city_states_from_disk() -> None:
        if not (
            city_swarm_lookup
            and curriculum_manager
            and curriculum_capture_dir
        ):
            return
        for slug, meta in city_swarm_lookup.items():
            pattern = os.path.join(curriculum_capture_dir, f"*{slug}*.state")
            try:
                matches = sorted(
                    glob.glob(pattern),
            ...
            curriculum_manager.add_state(
                matches[0],
                meta.get("label"),
                episodes=meta.get("episodes", cfg.auto_curriculum_capture_episodes),
                requires_badges=meta.get("requires_badges"),
                requires_story_flags=meta.get("requires_story_flags"),
            )


    _bootstrap_city_states_from_disk()
